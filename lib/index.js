...
function apply(ctx) {
  ...
  registerRoute('/dsh-extra/bg/search', async (body, send) => {
    ...
    const src = body && typeof body.source === 'string' ? body.source : 'wallhaven';
    if (src === 'safebooru') {
      try {
    ...
    if (src === 'konachan' || src.indexOf('custom:') === 0) {
      const domain = src === 'konachan' ? 'konachan.net' : src.slice('custom:'.length);
      if (!/^[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/.test(domain)) return send(200, { ok: false, error: '域名无效' });
      try {
        const tags = encodeURIComponent(raw ? raw.replace(/\s+/g, '+') : 'landscape');
        const url = 'https://' + domain + '/post.json?tags=' + tags + '&limit=30&page=' + page;
        const out = await curlText("curl -sS " + RETRY + "-A 'Mozilla/5.0' '" + url + "'");
        const parsed = JSON.parse(out);
        const items = (Array.isArray(parsed) ? parsed : []).map((x) => ({ id: String(x.id || ''), thumb: x.preview_url || '', full: x.sample_url || x.file_url || '', page: x.id ? ('https://' + domain + '/post/show/' + x.id) : '', resolution: (x.width && x.height) ? x.width + 'x' + x.height : '' })).filter((i) => i.thumb && i.full);
        if (items.length === 0) throw new Error('无结果');
        return send(200, { ok: true, items, lastPage: page + (items.length >= 30 ? 1 : 0), total: 0, source: src });
      } catch (e) { return send(200, { ok: false, error: String((e && e.message) || e) }); }
    }
    try {
  ...
  async function installedPlugins() {
    try {
      ...
      const res = await shell.run(shell.resolve({ command: 'cat "$HOME/.dsh/profiles/desktop/package.json"', timeoutMs: 10000, stdoutMaxBytes: 65536 }));
      if (res.exitCode !== 0 || !res.stdout || !res.stdout.text) return [];
      const text = res.stdout.text.trim();
      const parsed = JSON.parse(text);
      const bundles = parsed && parsed.dsh && parsed.dsh.profile && Array.isArray(parsed.dsh.profile.bundles) ? parsed.dsh.profile.bundles : [];
      return bundles;
    } catch { return []; }
  }

  registerRoute('/dsh-extra/profile/status', async (_body, send) => {
    try {
      const credentials = ctx.get('credentials');
      const keys = []; const seen = new Set();
      if (credentials) {
      ...
      const deepKey = keys.find((k) => k.provider === 'deepseek');
      let account = null; let accountError = null;
      if (deepKey && credentials) {
        try { const resolved = await credentials.resolve(deepKey.ref); if (resolved && resolved.value) account = await deepseekAccount(resolved.value); } catch (e) { accountError = String((e && e.message) || e); }
      }
      let workspaceCount = null; let sessionCount = null; let skillsCount = null; let presetsCount = null;
      const wr = ctx.get('workspaceRegistry'); if (wr) { try { workspaceCount = (await wr.list()).length; } catch {} }
      const sessions = ctx.get('sessions'); if (sessions) { try { sessionCount = sessions.list().length; } catch {} }
      const skills = ctx.get('skills'); if (skills) { try { skillsCount = (await skills.list()).length; } catch {} }
      const presets = ctx.get('agentPresets'); if (presets) { try { presetsCount = (await presets.list()).length; } catch {} }
      const plugins = await installedPlugins();
      return send(200, { keys, account, accountError, workspaceCount, sessionCount, skillsCount, presetsCount, plugins, error: null });
    } catch (e) { return send(200, { keys: [], account: null, accountError: null, workspaceCount: null, sessionCount: null, skillsCount: null, presetsCount: null, plugins: [], error: String((e && e.message) || e) }); }
  });

  registerRoute('/dsh-extra/profile/reveal', async (body, send) => {
    const ref = body && typeof body.ref === 'string' ? body.ref : '';
    try {
      const credentials = ctx.get('credentials');
      if (credentials === undefined) return send(200, { ok: false, error: '凭证服务不可用' });
  ...
  const PROFILE_PAGES = { topup: 'https://platform.deepseek.com/top_up', billing: 'https://platform.deepseek.com/transactions' };
  registerRoute('/dsh-extra/profile/openPage', async (body, send) => {
    const page = body && typeof body.page === 'string' ? body.page : 'topup';
    const url = PROFILE_PAGES[page] || PROFILE_PAGES.topup;
    try {
      const shell = ctx.get('shell');
      if (shell === undefined) return send(200, { ok: false, error: 'shell 服务不可用' });
      const res = await shell.run(shell.resolve({ command: 'open "' + url + '"', timeoutMs: 10000, stdoutMaxBytes: 8192 }));
      return send(200, res.exitCode === 0 ? { ok: true } : { ok: false, error: '打开失败' });
    } catch (e) { return send(200, { ok: false, error: String((e && e.message) || e) }); }
  });
