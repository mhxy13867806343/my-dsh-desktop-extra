  ...
  ctx.effect(() => {
    ...
    const disposeUpdate = slots.inject('sidebar.footer.action', () => slots.register(
      ...
      (props) => {
        ...
        React.useEffect(() => { let alive = true; host.call('desktopUpdate.status', {}).then((r) => { if (!alive || !r) return; setState((s) => ({ ...s, current: r.currentVersion, latest: r.latestVersion, upToDate: r.upToDate, updateAvailable: r.updateAvailable, error: r.error })); }).catch((e) => { if (alive) setState((s) => ({ ...s, error: String((e && e.message) || e) })); }); return () => { alive = false; }; }, []);
        ...
        const onCheck = () => { if (state.checking) return; setState((s) => ({ ...s, checking: true, error: null, notice: null })); host.call('desktopUpdate.check', {}).then((r) => { if (r) setState((s) => ({ ...s, checking: false, notice: r.message || null, error: r.error || null })); }).catch((e) => setState((s) => ({ ...s, checking: false, error: String((e && e.message) || e) }))); };
        const selectTheme = (id) => { if (theme) { try { theme.setTheme(id); } catch (e) { setState((s) => ({ ...s, error: String((e && e.message) || e) })); } } setThemeOpen(false); };
        const openLink = (url) => { closeMenus(); host.call('desktopUpdate.openUrl', { url }).catch((e) => setState((s) => ({ ...s, error: String((e && e.message) || e) }))); };
    ...
    const disposeAccount = slots.inject('settings.general.item', () => slots.register(
      ...
      () => {
        ...
        React.useEffect(() => { host.call('account.status', {}).then(applyResult).catch((e) => setState((s) => ({ ...s, loading: false, error: String((e && e.message) || e) }))); }, []);
        const refresh = () => { host.call('account.status', {}).then(applyResult).catch((e) => setState((s) => ({ ...s, error: String((e && e.message) || e) }))); };
        const toggleReveal = (ref) => { if (state.revealed[ref]) { setState((s) => ({ ...s, revealed: { ...s.revealed, [ref]: null } })); return; } host.call('account.reveal', { ref }).then((r) => { if (r && r.ok) setState((s) => ({ ...s, revealed: { ...s.revealed, [ref]: r.value } })); else if (r) setState((s) => ({ ...s, error: r.error || '无法读取 Key' })); }).catch((e) => setState((s) => ({ ...s, error: String((e && e.message) || e) }))); };
    ...
    const disposeBg = slots.inject('sidebar.footer.action', () => slots.register(
      ...
      (props) => {
        ...
        const setBackground = (item) => {
          ...
          host.call('bg.set', { url: item.full, name }).then((r) => {
            if (r && r.ok && r.dataUrl) {
        ...
        const download = (item) => {
          ...
          setError(null); setBusy(true); setBusyId(item.id); startProgress();
          host.call('bg.download', { url: item.full, name: safe }).then((r) => { if (r && r.ok) markDownloaded(item.id); else if (r) setError(r.error || '下载失败'); }).catch((e) => setError(String((e && e.message) || e))).finally(() => { setBusy(false); setBusyId(null); stopProgress(true); });
        };
        const search = (q, pg) => {
          const text = typeof q === 'string' ? q : query; const p = pg || 1;
          setQuery(text); setPage(p); setLoading(true); setError(null);
          host.call('bg.search', { query: text, page: p }).then((r) => { if (!r) return; if (r.ok) { setItems(r.items || []); setLastPage(r.lastPage || p || 1); setAppliedId(null); } else setError(r.error || '搜索失败'); }).catch((e) => setError(String((e && e.message) || e))).finally(() => setLoading(false));
        };
        ...
        const onOpen = () => { setOpen(true); setError(null); setView('online'); if (items.length === 0) search('anime', 1); let cfg = null; try { cfg = storeGet(); } catch {} setCurrentName(cfg && cfg.name ? cfg.name : ''); setHistory(historyGet()); setDownloaded(dlGet()); setCats(catsGet()); };
        ...
        return React.createElement(React.Fragment, null,
          ...
          ctxMenu ? React.createElement('div', { className: 'duiBgCtx', style: { left: Math.min(ctxMenu.x, window.innerWidth - 190), top: Math.min(ctxMenu.y, window.innerHeight - 130) } },
            ...
            React.createElement('button', { type: 'button', className: 'duiBgCtxItem', onClick: () => { host.call('bg.openUrl', { url: ctxMenu.item.page || ctxMenu.item.full }).catch(() => {}); setCtxMenu(null); } }, '打开链接')) : null);
      }
    ));
