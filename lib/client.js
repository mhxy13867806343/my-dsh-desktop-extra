    ...
    const disposeBg = slots.inject('sidebar.footer.action', () => slots.register(
      ...
      (props) => {
        ...
        const search = (q, pg) => {
          ...
          call('bg/search', { query: text, page: p }).then((r) => { if (!r) return; if (r.ok) { setItems(r.items || []); setLastPage(r.lastPage || p || 1); setAppliedId(null); } else setError(r.error || '搜索失败'); }).catch((e) => setError(String((e && e.message) || e))).finally(() => setLoading(false));
        };
